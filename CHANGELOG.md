...
- Internal API modularization: moved axis formatter policies/configs into `src/api/axis_config.rs`, axis-label formatting/quantization helpers into `src/api/axis_label_format.rs`, axis tick/window helpers into `src/api/axis_ticks.rs` and `src/api/data_window.rs`, interaction validation into `src/api/interaction_validation.rs`, label-cache types/logic into `src/api/label_cache.rs`, and validation routines into `src/api/validation.rs` with unchanged public re-exports.
